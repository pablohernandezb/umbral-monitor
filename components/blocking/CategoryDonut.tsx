// components/blocking/CategoryDonut.tsx
'use client';

import { useMemo } from 'react';
import { useTranslation } from '@/i18n';
import { PieChart, Pie, Cell, ResponsiveContainer, Tooltip } from 'recharts';
import { CATEGORY_COLORS } from './blocking-utils';
        ...
        {chartData.map((d) => (
          ...
          >
            <span
              ...
              style={{ background: CATEGORY_COLORS[d.name] || '#555' }}
            />
            {d.label} ({d.value})
          </button>
        ))}
      ...
      <ResponsiveContainer width="100%" height="100%">
        <PieChart>
          <Pie
            ...
            dataKey="value"
            cursor="pointer"
            onClick={(_, idx) => onCategoryClick(chartData[idx].name)}
          >
            {chartData.map((d, i) => (
              <Cell
                key={d.name}
                fill={CATEGORY_COLORS[d.name] || '#555'}
                opacity={
                  activeCategory && activeCategory !== d.name ? 0.25 : 1
                }
                stroke="transparent"
              />
            ))}
